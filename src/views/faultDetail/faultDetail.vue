<template>
  <div class="contents lingBut">
      ...
      <div class="detailBox">
        ...
        <div class="b_datas">
          <div class="words imageBox Flex">
            ...
            <!-- <div class="words m_T">提交时间：
              <span v-if="faultObj.repairState==0">{{faultObj.dispatchTime}}</span>
              <span v-if="faultObj.repairState==3">{{faultObj.handleTime}}</span>
              <span v-if="faultObj.repairState==6">{{faultObj.toAuditTime}}</span>
              <span v-if="faultObj.repairState==9">{{faultObj.auditTime}}</span>
            </div> -->
          </div>
    ...
    <div class="fix" v-if="faultObj.repairState==0||faultObj.repairState==3">
      <el-button type="primary" v-if="faultObj.repairState==0" @click="start">
        开始处理
      </el-button>
      <el-button type="primary" v-if="faultObj.repairState==3" @click="audit">
        请求验收
      </el-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    detail() {
      this.fileList = []
      http(api.detail, { repairId: this.repairId }).then(res => {
        this.faultObj = res;
        this.type = this.faultObj.repairType
        this.faultObj.repairFileList.forEach((element, index) => {
          // element.fileContent.split(',')
